...
export default function LeadGenWorkspace() {
  ...
  const [notifications, setNotifications] = useState([
    {
      id: 1,
      text: "Google Maps pipeline finished successfully.",
      time: "10m ago",
      read: false,
    },
    {
      id: 2,
      text: "Enriched CBSE schools list in Adyar.",
      time: "1h ago",
      read: true,
    },
    {
      id: 3,
      text: "Google Sheets sync completed.",
      time: "2h ago",
      read: true,
    },
  ]);
  ...
  const connectWebSocket = (leadsBeforeScrape: any[]) => {
    ...
    ws.onmessage = async (event) => {
      ...
      if (
        cleanMsg.includes("Starting Automated School Lead") ||
        cleanMsg.includes("Starting Google Maps search")
      ) {
        setCurrentStep(1);
      } else if (
        cleanMsg.includes("Commencing lead enrichment") ||
        cleanMsg.includes("Enriching Lead")
      ) {
        setCurrentStep((prev) => Math.max(prev, 2));
      } else if (
        cleanMsg.includes("Check Social Media Activity") ||
        cleanMsg.includes("checking social")
      ) {
        setCurrentStep((prev) => Math.max(prev, 3));
      } else if (
        cleanMsg.includes("Saving results") ||
        cleanMsg.includes("Syncing database")
      ) {
        setCurrentStep((prev) => Math.max(prev, 4));
      }

      if (
        msg.includes("finished successfully!") ||
        ...
        setNotifications((prev) => [
          {
            id: Date.now(),
            text: `Scraper finished for "${typeInput}" in "${areaInput}"`,
            time: "Just now",
            read: false,
          },
          ...prev,
        ...
        const newLeads = updatedLeads.filter(
          (l: any) =>
            !leadsBeforeScrape.some(
              (old: any) => old.school_name === l.school_name,
            ),
        );
        ...
        const runLeads =
          newLeads.length > 0
            ? newLeads
            : updatedLeads.filter((l: any) =>
                l.area_name.toLowerCase().includes(areaInput.toLowerCase()),
              );
    ...
    try {
      ...
      const interval = setInterval(() => {
        ...
        if (step === 1) {
          setLogs((prev) => [
            ...prev,
            "> [JOB INIT] Launching Playwright browser worker process...",
          ]);
        } else if (step === 2) {
          setLogs((prev) => [
            ...prev,
            `> Navigating Google Maps query: https://www.google.com/maps/search/${typeInput}+in+${areaInput}`,
          ]);
        } else if (step === 3) {
          setLogs((prev) => [
            ...prev,
            "> Found 3 matching schools in results sidebar. Commencing detail parsing...",
          ]);
        } else if (step === 4) {
          setLogs((prev) => [
            ...prev,
            "> Scraped School Name: Zion Matriculation School",
          ]);
          setLogs((prev) => [
            ...prev,
            "> Evaluating website Zion Matriculation School via Gemini Vision...",
          ]);
        } else if (step === 5) {
          setLogs((prev) => [
            ...prev,
            "> Gemini Web Audit: Redesign - Website loads slowly and relies on old flash templates.",
          ]);
        } else if (step === 6) {
          setLogs((prev) => [
            ...prev,
  ...
  const handleDeleteAllLeads = async () => {
    if (
      !confirm(
        "Are you sure you want to delete ALL leads from the database? This cannot be undone.",
      )
    )
      return;
    setLeads([]);
    ...
    try {
      ...
      if (res.ok) {
        setNotifications((prev) => [
          {
            id: Date.now(),
            text: "Successfully synchronized database leads to Google Sheets.",
            time: "Just now",
            read: false,
          },
          ...prev,
      ...
      setNotifications((prev) => [
        {
          id: Date.now(),
          text: "Successfully synchronized database leads to Google Sheets (Simulated).",
          time: "Just now",
          read: false,
        },
        ...prev,
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    if (!signupUsername || !signupPassword) {
      setLoginError(
        "Please enter both username and password for registration.",
      );
      return;
    }
    ...
    try {
      ...
      setLoginError(
        err.message || "Registration failed. Please check connection.",
      );
    } finally {
  ...
  const activeSocialMedia = leads.filter(
    (l) => l.social_media === "Active",
  ).length;
  const needsRedesign = leads.filter(
    (l) => l.appearance === "Redesign" || l.appearance === "Outdated",
  ).length;
  const noWebsite = leads.filter(
    (l) =>
      l.appearance === "Fresh" || !l.website_url || l.website_url === "None",
  ).length;
  const readyToContact = leads.filter(
    (l) =>
      (l.appearance === "Redesign" || l.appearance === "Fresh") &&
      l.contact_number,
  ).length;
  ...
  const filteredLeads = leads.filter((lead) => {
    const matchesSearch =
      ...
      (lead.search_area &&
        lead.search_area.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (lead.address &&
    ...
    const matchesAtmosphere =
      ...
      lead.atmosphere === filterAtmosphere;

    const matchesAppearance =
      filterAppearance === "All Appearances" ||
      lead.appearance === filterAppearance;
  ...
  const funnelData = [
    ...
    { name: "Ready to contact", value: readyToContact },
    {
      ...
      value: leads.filter((l) => l.status === "Email Sent").length,
    },
    {
      name: "Replied / Hot",
      value: leads.filter(
        (l) => l.status === "Replied" || l.status === "Meeting Booked",
      ).length,
    },
  ];
  ...
  if (!isAuthenticated) {
    return (
      <div
        className={`relative min-h-screen flex items-center justify-center font-sans overflow-hidden select-none ${
          isDarkMode ? "bg-zinc-950 text-white" : "bg-[#F5F7FA] text-[#111827]"
        }`}
      >
        ...
        <div
          className={`max-w-sm w-full border rounded-2xl p-7 shadow-2xl space-y-6 relative z-10 mx-4 transition-all duration-300 ${
            isDarkMode
        ...
        >
          <div className="text-center space-y-2">
            ...
            <h2 className="text-lg font-black uppercase tracking-wider text-[#00637C]">
              LeadFlow CRM
            </h2>
  ...
  return (
    <div
      className={`flex h-screen overflow-hidden font-sans ${isDarkMode ? "bg-zinc-950 text-zinc-100" : "bg-[#F5F7FA] text-[#111827]"}`}
    >
      {/* Sidebar Navigation */}
      <aside
        className={`w-64 border-r flex flex-col justify-between ${isDarkMode ? "bg-zinc-900 border-zinc-800" : "bg-white border-[#E2E8F0]"}`}
      >
        <div>
          {/* Sidebar Header */}
          <div
            className={`p-6 flex items-center gap-3 border-b ${isDarkMode ? "border-zinc-800" : "border-[#E2E8F0]"}`}
          >
            ...
            <div>
              <span
                className={`font-bold text-lg tracking-tight block ${isDarkMode ? "text-white" : "text-[#00637C]"}`}
              >
                LeadFlow
              </span>
        ...
        <div className="p-4 border-t border-[#E2E8F0] dark:border-zinc-800 space-y-3">
          <div
            className={`p-3.5 rounded-xl border ${isDarkMode ? "bg-zinc-950/40 border-zinc-800" : "bg-[#f8faf9] border-[#E2E8F0]"}`}
          >
            ...
            <div className="mt-2.5 flex items-center justify-between text-xs font-semibold">
              ...
              <span className={isDarkMode ? "text-teal-400" : "text-[#00637C]"}>
                {totalLeads}
              </span>
            </div>
            <div className="mt-1.5 flex items-center justify-between text-xs font-semibold">
              ...
              <span className={isDarkMode ? "text-teal-400" : "text-[#00637C]"}>
                {leads.filter((l) => l.appearance).length}
              </span>
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Top Header */}
        <header
          className={`h-16 border-b flex items-center justify-between px-8 select-none ${isDarkMode ? "bg-zinc-900 border-zinc-800" : "bg-white border-[#E2E8F0]"}`}
        >
            ...
            <div className="relative">
              ...
              {isNotificationsOpen && (
                <>
                  ...
                  ></div>
                  <div
                    className={`absolute right-0 mt-2 w-80 rounded-2xl border shadow-xl p-4 space-y-3 z-40 select-none ${
                      isDarkMode
                  ...
                  >
                    <div className="flex justify-between items-center pb-2 border-b border-zinc-100 dark:border-zinc-850">
                      <span className="text-xs font-black uppercase tracking-wider text-[#00637C]">
                        System Alerts
                      </span>
                      <button
                        onClick={() => {
                    ...
                    <div className="space-y-2.5 max-h-56 overflow-y-auto">
                      {notifications.length === 0 ? (
                        <p className="text-[10px] text-zinc-400 font-semibold text-center py-4">
                        ...
                        notifications.map((n) => (
                          ...
                          >
                            <p
                              className={`text-[10px] leading-normal font-semibold ${n.read ? "text-zinc-400" : "text-zinc-700 dark:text-zinc-200"}`}
                            >
                              {n.text}
                            </p>
                            <span className="text-[8px] text-zinc-400 font-bold block mt-1 uppercase tracking-widest">
                              {n.time}
                            </span>
                          </div>
                        ))
            ...
            <div className="relative">
              ...
              {isProfileOpen && (
                <>
                  ...
                  ></div>
                  <div
                    className={`absolute right-0 mt-2 w-64 rounded-2xl border shadow-xl p-4 space-y-4 z-40 select-none text-left ${
                      isDarkMode
                        ? "bg-zinc-900 border-zinc-850 text-white"
                        : "bg-white border-[#E2E8F0] text-[#111827]"
                    }`}
                  >
                    ...
                    <div className="border-t border-zinc-100 dark:border-zinc-850 pt-3.5 space-y-2">
                      <div className="flex justify-between items-center text-[9px] font-bold text-zinc-400 uppercase tracking-wider">
                        ...
                        <span className="text-emerald-500 lowercase tracking-normal">
                          connected
                        </span>
                      </div>
        ...
        <div className="flex-1 overflow-y-auto p-8">
          ...
          {activeTab === "discovery" && (
            <div className="relative flex-1 flex flex-col items-center justify-center min-h-[75vh] w-full px-4 overflow-hidden select-none">
              ...
              {isSearching ? (
                ...
                <div
                  className={`max-w-xl w-full border rounded-2xl p-8 shadow-[0_10px_40px_rgba(0,0,0,0.04)] space-y-7 relative z-10 ${
                    isDarkMode
                ...
                >
                  <div className="text-center pb-2 border-b border-zinc-100 dark:border-zinc-850">
                    <h3 className="text-sm font-black uppercase tracking-wider text-[#00637C]">
                      AI Lead enrichment pipeline
                    </h3>
                    <p className="text-[10px] text-zinc-400 font-bold mt-1">
                      Playwright crawler is active on maps listings
                    </p>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <span
                            className={`text-xs font-bold ${currentStep === 1 ? "text-[#00637C]" : currentStep > 1 ? "text-zinc-500" : "text-zinc-400"}`}
                          >
                            1. Google Maps Parsing
                          </span>
                          {currentStep === 1 && (
                            <Loader2 className="h-3 w-3 animate-spin text-[#00637C]" />
                          )}
                        </div>
                        <p className="text-[10px] text-zinc-400 mt-0.5 font-medium">
                          Extracting names, domains, and phone numbers from live
                          Maps feeds.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <span
                            className={`text-xs font-bold ${currentStep === 2 ? "text-[#00637C]" : currentStep > 2 ? "text-zinc-500" : "text-zinc-400"}`}
                          >
                        ...
                        <p className="text-[10px] text-zinc-400 mt-0.5 font-medium">
                          Analyzing design responsiveness, layout structure, and
                          red-flags via Gemini.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <span
                            className={`text-xs font-bold ${currentStep === 3 ? "text-[#00637C]" : currentStep > 3 ? "text-zinc-500" : "text-zinc-400"}`}
                          >
                        ...
                        <p className="text-[10px] text-zinc-400 mt-0.5 font-medium">
                          Scanning Facebook, LinkedIn, and Instagram for recent
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <span
                            className={`text-xs font-bold ${currentStep === 4 ? "text-[#00637C]" : "text-zinc-400"}`}
                          >
                        ...
                        <p className="text-[10px] text-zinc-400 mt-0.5 font-medium">
                          Committing enriched records to MySQL database &
                          syncing sheets.
                        </p>
                      </div>
                    </div>
                ...
                <div className="max-w-3xl w-full space-y-6 relative z-10 animate-fade-in">
                  <div
                    className={`border rounded-2xl p-6 shadow-[0_10px_40px_rgba(0,0,0,0.03)] space-y-5 ${
                      isDarkMode
                        ? "bg-zinc-900 border-zinc-800 text-white"
                        : "bg-white border-[#E2E8F0] text-[#111827]"
                    }`}
                  >
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 pb-3 border-b border-zinc-100 dark:border-zinc-850">
                      <div>
                        <h3 className="text-sm font-extrabold text-[#00637C] uppercase tracking-wider">
                          Google Maps Discovery Engine
                        </h3>
                        <p className="text-[10px] text-zinc-400 font-semibold mt-0.5">
                          Scrape new target leads directly from Google Maps and
                          enrich them automatically via AI.
                        </p>
                      </div>
                    </div>
                          ...
                          {isScanDepthOpen && (
                            <>
                              <div
                                ...
                                onClick={() => setIsScanDepthOpen(false)}
                              ></div>
                              <div
          ...
          {activeTab === "archive" && (
            <div className="space-y-6 animate-fade-in text-[#111827]">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {/* Total Schools */}
                <div
                  className={`border rounded-xl p-4 shadow-sm flex items-center justify-between transition-all hover:shadow-md ${isDarkMode ? "bg-zinc-900 border-zinc-800 text-white" : "bg-white border-[#E2E8F0] text-[#111827]"}`}
                >
                  <div>
                    ...
                    {totalPages > 1 && (
                      <div className="flex justify-between items-center pt-4 border-t border-[#E2E8F0] dark:border-zinc-800">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span
                            className={`text-[10px] font-bold px-2.5 ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}
                          >
          ...
          {activeTab === "config" && (
            <div className="max-w-2xl mx-auto space-y-6 animate-fade-in text-[#111827]">
              <div
                className={`border rounded-xl p-6 shadow-sm ${isDarkMode ? "bg-zinc-900 border-zinc-800 text-white" : "bg-white border-[#E2E8F0] text-[#111827]"}`}
              >
                <div className="mb-6">
                  <h3 className="text-md font-bold">API Key & Sync Settings</h3>
                  <p className="text-xs text-zinc-400 mt-1">
                    Manage secrets and connections. These keys are saved
                    directly into your local workspace `.env` file.
                  </p>
                </div>
          ...
          <div className="fixed inset-y-0 right-0 w-[480px] bg-white border-l border-[#E2E8F0] shadow-2xl z-50 flex flex-col animate-slide-in font-sans">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  ...
                  return (
                    <>
                      <div className="relative h-16 w-16 flex items-center justify-center shrink-0">
                        ...
                        <span className="absolute text-xs font-black text-[#111827]">
                          {score}%
                        </span>
                      </div>
                      <div>
              ...
              <div className="space-y-2 select-none">
                ...
                <div
                  className={`border rounded-xl p-4 shadow-sm space-y-3 ${isDarkMode ? "bg-zinc-900 border-zinc-800 text-white" : "bg-[#F5F7FA] border-[#E2E8F0] text-[#111827]"}`}
                >
                ...
                <div className="border border-[#E2E8F0] rounded-xl overflow-hidden bg-[#F5F7FA] font-mono text-[9px] shadow-sm">
                  ...
                  <div className="bg-white p-5 flex flex-col justify-center items-center text-center">
                    {selectedLead.website_url ? (
                      ...
                      <div className="space-y-1">
                        <div className="text-[11px] font-bold text-red-500 uppercase tracking-wide">
                          Domain Offline / Fresh Design
                        </div>
                        <div className="text-[10px] text-zinc-400">
                          No registered domain found in discovery records.
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-2">
                <div className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">
                  Social Presence (Last 30 Days)
                </div>
                <div className="border border-[#E2E8F0] rounded-xl p-4 bg-[#F5F7FA] shadow-sm space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-wider">
                      Facebook / Instagram scans
                    </span>
                    <span
                      className={`inline-block font-extrabold px-2 py-0.5 rounded text-[9px] text-white ${
              ...
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-2 gap-3 text-[10px] select-none">
                  ...
                  <div className="border border-[#E2E8F0] p-3 rounded-lg bg-white shadow-sm">
                    ...
                    <span
                      className={`font-bold block mt-0.5 ${selectedLead.atmosphere === "Good" ? "text-emerald-600" : "text-red-500"}`}
                    >
                      {selectedLead.atmosphere === "Good"
                        ? "✓ Looking Good"
                        : "✗ Needs Facelift"}
                    </span>
                  </div>
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-2">
                  {(() => {
                    const recs = [];
                    if (
                      selectedLead.appearance === "Fresh" ||
                      !selectedLead.website_url
                    ) {
                      recs.push(
                        "Propose brand new website starter kit (domain, landing page, hosting).",
                      );
                      recs.push(
                        "Target admissions traffic showcasing custom maps integration.",
                      );
                      recs.push(
                      ...
                      recs.push(
                        "Propose responsive Next.js portfolio redesign package.",
                      );
                      recs.push(
                        "Draft cold email target sequence focusing on design quality comparison.",
                      );
                    } else {
                      recs.push(
                        "Pitch search ads audit to optimize current website enrollment traffic.",
                      );
                      recs.push(
                        "Offer Facebook/Instagram management service (currently inactive).",
                      );
                      recs.push(
                        "Suggest conversion rate enhancement updates for current page.",
                      );
                    }
                    ...
                    return recs.map((rec, i) => (
                      <li
                        key={i}
                        className="flex gap-2 items-start text-[10px] font-semibold text-zinc-600 leading-normal"
                      >
      ...
      {isAddModalOpen && (
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <div
              className={`max-w-md w-full border rounded-2xl p-6 shadow-2xl space-y-4 animate-scale-in ${
                isDarkMode
                  ? "bg-zinc-900 border-zinc-800 text-white"
                  : "bg-white border-[#E2E8F0] text-[#111827]"
              }`}
